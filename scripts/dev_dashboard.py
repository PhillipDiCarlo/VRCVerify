...
# THE ONLY THREE VALUES BELOW THAT NAME A REAL FILE, so they are the only ones
# that could fail to exist. They used to point at `certs/` in the repository,
# which is gitignored and generated by scripts/gen_bot_api_certs.sh -- so the
# preview started on the machine of anybody who had run that script and nowhere
# else. On a fresh clone it died with "BOT_API_CLIENT_CERT points at ...,
# which is not a file", and three tests in tests/test_preview_tool.py went with
# it. That went unnoticed because the tests passed on every machine that had
# already generated certs, which was every machine that had ever run the bot.
# CI on a clean checkout is what found it (#297).
#
# Placeholders, not real certificates, because the preview never completes a
# TLS handshake: BOT_API_URL is a closed port and PreviewBotAPI answers every
# call in-process. config.py only asks whether the paths are readable files,
# which is the check that exists so the service fails at start-up with the
# file's name rather than at the first handshake with an error naming neither.
# Written outside the repository, beside the session database, so a preview
# leaves nothing behind in the working tree.
_PLACEHOLDER_CERTS = pathlib.Path(tempfile.gettempdir()) / "vrcverify-preview-certs"
_PLACEHOLDER_CERTS.mkdir(parents=True, exist_ok=True)
for _name in ("dashboard.pem", "dashboard.key", "ca.pem"):
    (_PLACEHOLDER_CERTS / _name).write_text(
        "# Not a certificate. scripts/dev_dashboard.py writes this so the\n"
        "# dashboard's config check has a readable file to find. The preview\n"
        "# never opens a TLS connection to anything.\n"
    )
...
os.environ.update(
    ...
    BOT_API_CLIENT_CERT=str(_PLACEHOLDER_CERTS / "dashboard.pem"),
    BOT_API_CLIENT_KEY=str(_PLACEHOLDER_CERTS / "dashboard.key"),
    BOT_API_CA=str(_PLACEHOLDER_CERTS / "ca.pem"),
