/* The theme toggle, copied byte for byte from site/theme.js (issue #170).
 *
 * A COPY, and for the same reason the stylesheet beside it is a copy: this
 * page exists to keep working when the apex Worker is not, so it cannot fetch
 * a script from it. tests/test_status_page.py asserts the two files are
 * identical below this header, so a change to one is a failing test rather
 * than two sites that toggle differently.
 *
