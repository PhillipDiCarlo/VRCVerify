...
3. **VRChat Group Inviter (vrc_group_inviter.py):**
   ...
   - Sends one member's invite on request, after checking where they already stand: an existing member, a waiting invite, or a pending join request all mean "nothing to send", each with its own answer. That check is best effort and can never block an invite — every failure of it falls through to attempting the invite anyway, because what `get_group_member` requires has never actually been measured and the design deliberately does not need to know. `create_group_invite` is where every authoritative answer comes from — 400 for an existing member, 403 for a recipient who will not take invites, 404 for a group that has gone **or** a member whose linked VRChat account no longer resolves. The last two are told apart by asking whether the group still exists, because they need opposite advice: re-verify, or go and find an admin. `confirm_override_block` is passed as **False** explicitly, because `vrchatapi` defaults it to `True` — omitting it would opt in to pushing invites past people who have blocked the group.
