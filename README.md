...
3. **VRChat Group Inviter (vrc_group_inviter.py):**
   ...
   - Sends one member's invite on request, after checking where they already stand: an existing member, a waiting invite, or a pending join request all mean "nothing to send", each with its own answer. That check needs the **optional** `group-members-viewall`, so it is skipped for groups that never granted it — the invite still goes out, and `create_group_invite`'s own 400 catches an existing member. `confirm_override_block` is passed as **False** explicitly, because `vrchatapi` defaults it to `True` — omitting it would opt in to pushing invites past people who have blocked the group.
