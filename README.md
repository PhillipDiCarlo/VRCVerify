...
3. **VRChat Group Inviter (vrc_group_inviter.py):**
   ...
   - Sends one member's invite on request. **Only two answers stop it:** they are already in the group, or the group has banned them. (`userblocked` refuses as well, but that is not a third policy — the invite would be answered 403 anyway, since `confirm_override_block` is always `False`, so refusing merely saves the call.) Membership is read first so those answers cost no invite, and so the two states that need *action* rather than a sentence can be recognized — see below. That check is best effort and can never block an invite — every failure of it falls through to attempting the invite anyway, and the design deliberately does not need it to succeed. `create_group_invite` is where every authoritative answer comes from — 400 for an existing member **or** one who already has an invite waiting, 403 for a recipient who will not take invites, 404 for a group that has gone **or** a member whose linked VRChat account no longer resolves, 409 for a member the group has banned. The 403 and 404 are told apart by asking whether the group still exists, because they need opposite advice: re-verify, or go and find an admin. The two 400s are told apart by their wording, which is the only place in the worker where a message is read rather than a status — being a member is finished, while a waiting invite is a nudge to check their notifications, and answering both with "already a member" sent people hunting through a group they had not joined. That match is anchored to the **end** of the sentence, because the part in front of it is the member's own display name; an unrecognized 400 keeps the older reading and says so in the log. The 409 needs no wording at all: VRChat names that case by status. `confirm_override_block` is passed as **False** explicitly, because `vrchatapi` defaults it to `True` — omitting it would opt in to pushing invites past people who have blocked the group.
     ...
     **A waiting invite is re-issued, not refused.** Dismissing an invite notification is not the same action as declining it, and only one is safe: declining clears the invite record and lands on `inactive`, while dismissing (the x, which is what people press when clearing a list) deletes what the member can see and leaves the record standing — so `create_group_invite` answers 400 "already invited" for ever. That stranded the member with nothing to accept while the bot told them an invite was waiting in notifications they had just cleared. So a press against a `invited` standing withdraws the stale invite and sends a fresh one. It is not a duplicate: VRChat holds exactly one invite per member, and this is that invite made visible again. Because the membership check is best effort, the same rescue also runs off VRChat's own answer: a 400 "already invited" from a member the check could not read is treated as that standing, withdrawn and re-sent once. Bounded to a single re-attempt — a second "already invited" is reported rather than retried — and both routes run the one withdraw-and-re-send rather than two implementations of it.

     **A pending join request is admitted, not queued.** An invite against someone who has applied to join admits them immediately rather than queueing. This used to be refused, to avoid cutting across a moderator's pending decision — but that had it backwards. An admin who configures this bot for their group has delegated exactly that decision to it, and a member who is verified 18+, in the Discord, and pressing the button is the population it was installed to admit. Leaving them in the queue refuses somebody the bot has already vouched for.
   ...
   - Leaves a group on request, which is the only other write it performs and follows the same rule as the join: it leaves the group the job names and nothing else, and the bot builds that job from its own stored lease. Already being out counts as success — a 404 means the seat is free either way, and reporting a failure would pin it behind a group that does not exist. The job carries a **reason** the worker echoes back unread: it leaves either way, but the bot has to tell "this server lapsed, free its seat" from "this admin changed group, leave the old one and change nothing else".
   - Reads a group's pending join requests a page at a time, and accepts or rejects one when a moderator presses Approve or Deny in Discord (#291). It never decides who gets in. Responding needs only `group-invites-manage`, the permission invite setup already checks (measured 2026-09-17), and the action is sent as a plain string because vrchatapi 1.21.0 serializes its enum model as `[object Object]`. Answering a request that is already gone is a 400 that is reported as handled elsewhere, not as a failure.
...
**Seats, and more than one invite account.** A VRChat account can be in 100 groups (200 with VRC+), which makes that cap the scarcest thing the feature has — and it is spent by *servers*, not members.

   - `INVITE_ACCOUNTS` is the roster (`usr_id|queue|seats`, comma-separated, seats optional). **Leave it unset while you run one account**: the bot builds a one-entry roster from `INVITE_VRCHAT_USER_ID` and `RABBITMQ_GROUP_INVITE_QUEUE`, so an existing deployment upgrades with no configuration change at all.
   - **Each account needs its own queue**, and its own `vrc-group-inviter` service. Two workers on one queue split its messages round-robin, so an invite for a group only the second account joined would be handed to the first — the same trap that made this worker take its own queue instead of sharing the checker's. Routing is therefore explicit: every job goes to the queue of the account holding that guild's seat.
   - A seat is **reserved when the account is assigned**, which is the moment the dashboard first names it to an admin — not when the bot finally joins. Counting only joined groups would let many simultaneous setups all be sent to the same nearly-full account. A reservation that never reaches a successful verification is released after `SEAT_RESERVATION_GRACE_SECONDS`.
   - **Each admin is told the account that will actually serve them** — theirs if they hold a seat, otherwise the one they would be assigned. Merely rendering the settings page never reserves anything. Naming a single global account instead would produce exactly the unexplained "not a member" failure the setup flow already warns about, for every server not assigned to the first account.
   - Guilds configured **before seat accounting existed** hold a seat with no row to say so, so the sweep backfills one — preferring the account the worker reported joining, which on a multi-account installation is the only thing that knows which account is really in the group. Without the backfill the lapse clock never starts and those occupied seats read as free.
   - Assignment is **least-loaded and sticky**. Least-loaded because the accounts are separate precisely so moderation action against one cannot take the others down; sticky because the account an admin was told to invite is the account that is in their group.
   - After `SEAT_LAPSE_GRACE_SECONDS` (90 days) without a subscription the bot **leaves the group and the seat returns to the pool** — but the server's configuration survives. `group_id` keeps its UNIQUE claim so nobody can take the group while they are away, and `verified_at` is kept, which is what makes `requireCode` stay `False`: a returning admin re-invites the bot and presses check, and never pastes the claim code back into their group description.
   - The seat is freed only when the worker confirms it is **actually out** of the group. Freeing it on the request would advertise capacity the bot is still occupying.
   - This is the one part of the feature that needs a scheduler. Everything else expires on read, deliberately — but nothing reads a lapsed guild's row, which is exactly what makes it lapsed, so `seat_sweep_task` is what notices. It never reclaims on an unanswered question: a billing lookup that failed leaves the seat alone, because reading a hiccup as "not paying" would evict paying customers from their own groups.
...
4. **Database Setup:**
  ...
  table is created automatically, with no manual migration needed.
   ...
   SELECT
     day AS "time",
     active_count AS "active servers",
     inaccessible_count AS "inaccessible servers"
   FROM server_membership_daily
   WHERE $__timeFilter(day)
   ORDER BY day;
   ```

  The first row is recorded after the updated bot starts. A day can be
